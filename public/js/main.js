//let data = [];
    ...
    */
    ...
    /*
    var draw = (dom_elm_id_name, info, vis_info) => {
	  // ...
    };
    ...
    // draw( "vis", handlers, edua.info(), edua.visInfo() );
    */
    ...
    var draw = (data = []) => {
      ...
      const createTextData = (data) => {
        data.forEach((element) => {
          if(element.posX === "9") {
            textData.push(
              {
                "posX": element.posX * 400 - 110,
                "posY": element.posY * 400 - 200,
                "title": element.name,
                "mode": element.mode,
            ...
            textData.push(
              {
                "posX": element.posX * 400 + 240,
                "posY": element.posY * 400,
                "title": element.name,
                "mode": element.mode,
                "timeLeft": element.timeLeft
              }
      ...
      modes
        .attr("x", function (d) { return d.posX + 160; })
        ...
        .attr("fill", function (d) {
          if (d.mode === "N") return "red"
          else if (d.mode === "S") return "orange"
          else if (d.mode === "I") return "blue"
          else if (d.mode === "O") return "green"
          else return "black"
        });
      }
      ...
      const drawResources = (drawResource, drawNoResource) => {
        data.forEach(element => {
          if(element.resourcesPresent === "true") {
            drawResource(
            element.posX,
            element.posY, 
            element.name, 
          ...
          else if (element.resourcesPresent === "false") {
            drawNoResource(
              element.posX,
              element.posY, 
              element.name, 
      ...
      const drawBuffer = (posX, posY, fill, name) => {
        ...
        .attr("d", paths[0].d2)
        .attr("id", function() { return name })
        .style("transform", `scaleX(${1.18}) scaleY(${1.1428}) translate(${337.5 * posX}px, ${350 * posY}px)`)
        .style("fill", fill)
      }

      const buffers = data.filter(element => {
        return element.name.includes("Buffer"
        )});
      ...
      const drawWorkStation = (posX, posY, fill, name) => {
        svg.selectAll("Station")
        .data(dataset)
        .enter()
        .append('path')
        .attr("class", "Station")
        .attr("d", paths[0].d1)
        .style("transform", `scale(${7.467}) translate(${53.6 * posX - 5}px, ${53.6 * posY}px)`)
        .style("fill", fill)
        .attr("id", function() { return name })
      }

      const workStations = data.filter(element => {
        return element.name.includes("Station"
        )});
      ...
      const drawWorkStations = ((drawWorkStation, drawProgressBar) => {
        workStations.forEach(element => {
          ...
          drawProgressBar(
            5 / scale,
            2.2, element.posY / scale - 0.07,
            element.posX - 0.2,
            "white",
            "outline: 5px solid black",
            0)
          drawProgressBar(
            (4 * element.progressBarPercent / 100) * 1.23 / scale,
            2.2,
            element.posY / scale - 0.07,
            element.posX - 0.2,
            "green",
            "outline: 5px solid black",
            5 - element.progressBarPercent / 100 * 1.23 * 4
          )
          // drawState(
          //   element.posX,
          //   element.posY,
      ...
      const drawArrowLine = (posX1, posY1, posX2, posY2, triangle) => {
        ...
        svg.append("line")
          .attr("x1", function () {
            return xScale(posX1 * 10 + transitionX1);
          })
          .attr("y1", function () {
            return xScale(posY1 * 10 + transitionY1);
          })
          .attr("x2", function () {
            return xScale(posX2 * 10 + transitionX2);
          })
          .attr("y2", function () {
            return xScale(posY2 * 10 + transitionY2);
          })
          .attr("stroke-width", 15)
          .attr("stroke", "black")
      ...
      drawArrowLines = (drawArrowLine) => {
        data.forEach((element) => {
          ...
          if(element.output !== "") {
            const outputs = JSON.parse(element.output)
            for(let i = 0 ; i < outputs.length ; i++) {
              let posX = parseInt(data[outputs[i] - 1].posX);
              let posY = parseInt(data[outputs[i] - 1].posY);
                   drawArrowLine(
                     parseInt(element.posX),
                     parseInt(element.posY),
                     posX,
                     posY,
                     "url(#triangle)"
                   )
      ...
      .on("click", function(d) { alert(this.id); })
